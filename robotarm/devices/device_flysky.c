...
//定义无线模块的管脚
#define GIO PBin(12)  //只输入
#define MDI PBout(13)  //双模式
#define SCK PBout(14)  //只输出
#define CSN PBout(15)  //只输出

//IO方向设置
#define MDI_IN()  {GPIOB->CRH&=0XFF0FFFFF;GPIOB->CRH|=8<<20;}
#define MDI_OUT() {GPIOB->CRH&=0XFF0FFFFF;GPIOB->CRH|=3<<20;}
#define MDI_READ() PBin(13)

u8 TR0=0;
#define EA_ON()  {TIM_ITConfig(  TIM7,TIM_IT_Update ,ENABLE ); TR0=1;}
#define EA_OFF() {TIM_ITConfig(  TIM7,TIM_IT_Update ,DISABLE );TR0=0;}
...
void RC_GPIO_Init()
{
 ...
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;				 //MDI 端口配置
 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
 GPIO_Init(GPIOB, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB12
	
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;				 //BIND 端口配置
 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
 GPIO_Init(GPIOA, &GPIO_InitStructure);					 //根据设定参数初始化GPIOA11
	
 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;				 //SCK  端口配置
...
void TIM7_IRQHandler(void)   //加跳定时器  20ms
{
